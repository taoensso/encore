...
#?(:clj
   (defmacro binding
     ...
     {:added "Encore v3.98.0 (2024-03-28)"
      :style/indent 1}
     [bindings & body]
     (if (:ns &env)
...
(defn format-inst-fn
  ...
  #?(:cljs
     ...
     ([{:keys [formatter]
      ...
      (instance! java.time.format.DateTimeFormatter formatter) ; Thread safe
...
#?(:cljs
   (do
     (def* pr      "Identical to `core/pr`."      {:added "Encore v3.98.0 (2024-03-28)"} cljs.core/pr)
     (def* prn     "Identical to `core/prn`."     {:added "Encore v3.98.0 (2024-03-28)"} cljs.core/prn)
     (def* print   "Identical to `core/print`."   {:added "Encore v3.98.0 (2024-03-28)"} cljs.core/print)
     (def* println "Identical to `core/println`." {:added "Encore v3.98.0 (2024-03-28)"} cljs.core/println))
...
#?(:clj
   (defn- refreshing-cache [f1]
     (let [cache_ (latom nil) ; ?[promise udt]
       ...
       (fn [^long cache-msecs timeout-msecs timeout-val]
              ...
              (if-let [[p ^long t0] (cache_)]
                ...
                (let [p (promise)]
                  (when (compare-and-set! cache_ nil [p t1]) ; First call
                    ...
                    (threaded (p (f1 timeout-val))))
                  (recur true))))))))))

#?(:clj
   (let [f1 (fn [fallback] (catching (.getHostAddress (java.net.InetAddress/getLocalHost)) _ (force fallback)))
...
#?(:clj
   (let [f1 (fn [fallback] (catching (.getHostName (java.net.InetAddress/getLocalHost)) _ (force fallback)))
         f3 (refreshing-cache f1)]
...
(let [map-like? #(or (map? %) (record? %))]
  (defn- pw
    "Custom, faster post-walk."
    [x f]
    (cond
      (map-like? x) (do #_f       (reduce-kv (fn [acc k v] (assoc acc (pw k  f) (pw v f))) {}        x))
      (seq?      x) (do #_f (rseq (reduce    (fn [acc  in] (conj  acc (pw in f)))          []        x)))
      (coll?     x) (do #_f       (reduce    (fn [acc  in] (conj  acc (pw in f)))          (empty x) x))
      :else         (f x))))

(defn pr-json
  #?(:cljs "Returns given Cljs argument as JSON string."
     :clj
     "Experimental, subject to change without notice.
  ...
  {:added "Encore v3.98.0 (2024-03-28)"
   :tag #?(:clj 'String :cljs 'string)}
  [x]
  #?(:cljs (.stringify js/JSON (clj->js x :keyword-fn as-qname))
     :clj
     (compile-if (do (require '[jsonista.core :as jsonista]) true #_false)
       (jsonista/write-value-as-string x)
       (compile-if (do (require '[cheshire.core :as cheshire]) true #_false)
         (let [x (pw x #(if (inst? %) (format-inst %) %))]
           (cheshire/generate-string x))
         (compile-if (do (require '[clojure.data.json :as data-json]) true #_false)
           (let [x (pw x #(or (as-?qname %) %))]
             (data-json/write-str x
               :escape-unicode       false
               :escape-js-separators false
               :escape-slash         false))
           (throw
             (ex-info "[encore/pr-json] No supported JSON library found"
               {:checked '[metosin/jsonista cheshire org.clojure/clojure.data]})))))))

(comment
  (pr-json {:k [:k "s" 1 1.2 {"s" :a/b} a-utf8-str (as-inst 1711619032283)]})
  "{\"k\":[\"k\",\"s\",1,1.2,{\"s\":\"a/b\"},\"Hi ಬಾ ಇಲ್ಲಿ ಸಂಭವಿಸ 10\",\"2024-03-28T09:43:52.283Z\"]}")

(defn read-json
  "Experimental, subject to change without notice.
  Reads given JSON string to return a Clj/s value. Prefer directly using a lib
  with the opts of your choice when you have specific needs!"
  {:added "Encore v3.98.0 (2024-03-28)"}
  ([         s] (read-json false s))
  ([kw-keys? s]
   (cond
        ...
        (compile-if (do (require '[jsonista.core :as jsonista]) true #_false)
          (jsonista/read-value s
            (if kw-keys?
              ...
              jsonista/default-object-mapper))

          (compile-if (do (require '[cheshire.core :as cheshire]) true #_false)
            (cheshire/parse-string s kw-keys?)

            (compile-if (do (require '[clojure.data.json :as data-json]) true #_false)
              (if kw-keys?
                (data-json/read-str s :key-fn keyword)
                (data-json/read-str s))

              (throw
                (ex-info "[encore/read-json] No supported JSON library found"
                  {:checked '[metosin/jsonista cheshire org.clojure/clojure.data]})))))))))

(comment
  (read-json "{\"k\":[\"k\",\"s\",1,1.2,{\"s\":\"a/b\"},\"Hi ಬಾ ಇಲ್ಲಿ ಸಂಭವಿಸ 10\",\"2024-03-28T09:43:52.283Z\"]}")
  {"k" ["k" "s" 1 1.2 {"s" "a/b"} "Hi ಬಾ ಇಲ್ಲಿ ಸಂಭವಿಸ 10" "2024-03-28T09:43:52.283Z"]})
...
(deprecated
  ...
  (def* ^:no-doc println-atomic "Prefer `println`."       {:deprecated "Encore v3.98.0 (2024-03-28)"} println)

  #?(:cljs (def* ^:no-doc ajax-lite "Prefer `ajax-call`." {:deprecated "Encore v3.74.0 (2023-11-06)"} ajax-call))
  #?(:cljs (def* ^:no-doc now-dt    "Prefer `now-inst`."  {:deprecated "Encore v3.98.0 (2024-03-28)"} now-inst))
  #?(:clj
     (do
  ...
  #?(:clj
     (defmacro ^:no-doc deftype-print-methods "Prefer `def-print`."
       {:deprecated "Encore v3.98.0 (2024-03-28)"}
       [& types]
       `(do
          ~@(map
              (fn [type]
